...
from models.phase1_partial3D import FasterNet#flare_result_phase1_OriginPartial_Nospacing
...
# Argument Parsingn
#/home/plyu/Projects/dataset/Dataset050_Flare2023/'
parser = argparse.ArgumentParser()
parser.add_argument('--data_path', type=str, default='', help='data directory')
parser.add_argument('--exp', type=str,  default='', help='model save path')
parser.add_argument('--stage1_model_weight', type=str,  default='', help='initial save path of phase1')
parser.add_argument('--stage2_model_weight', type=str,  default='', help='initial save path of phase2')
parser.add_argument('--max_iteration', type=int,  default=12290, help='maximum iteration number to train')
parser.add_argument('--batch_size', type=int, default= 8, help='batch_size per gpu')
parser.add_argument('--base_lr', type=float,  default=3.5e-4, help='starting learning rate') 
parser.add_argument('--deterministic', type=bool,  default=True, help='whether use deterministic training')
parser.add_argument('--seed', type=int,  default=50, help='random seed')
parser.add_argument('--gpu', type=list,  default=[0,1,2,3], help='GPU to use')
parser.add_argument('--dataset', type=str, default='flare2024', help='dataset') #'BTCV', coronary, flare2023
parser.add_argument('--phase', type=str, default='phase_2', help='dataset') #'BTCV', coronary, flare2023
...
if args.phase == 'phase_1':
    ...
    if args.stage2_model_weight:
        net = torch.nn.DataParallel(net, device_ids=gpus)   

        net.load_state_dict(torch.load(args.stage2_model_weight))

        # ckpt=torch.load(args.stage2_model_weight, map_location='cpu')
        # net_dict=net.state_dict()
        # pretrained_dict={k:v for k,v in ckpt.items() if k in net_dict and (v.shape==net_dict[k].shape) }
...
# ce_weight = torch.tensor([1,1,1,1,1.5,1,1.2,2,2,5,2,1,3,1], device='cuda').float()
# DiceCEloss = DiceCELoss(softmax=True,include_background = True, to_onehot_y = True,lambda_dice=1.0, lambda_ce=1.0, ce_weight = ce_weight) 
...
scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=int(max_epoch), T_mult=1,eta_min=1e-5)
...
while epoch_num <= max_epoch:
    ...
    if  epoch_num % ( 2 if epoch_num<round(max_epoch/2) else 1) == 0 or global_step == max_iterations:
        net.eval()
        patch_size =  (96,96,96)
        sw_batch_size = 1
        with torch.no_grad():
        ...
        if DSC > best_dice or ((DSC == best_dice) and  (HD95 <= corresponding_HD)) :
                ...
                save_best_mode_path = os.path.join(snapshot_path, "best_metric_model_"+ str(best_dice_global_step)+'_'+str(int(best_dice*10000))  + '.pth')
