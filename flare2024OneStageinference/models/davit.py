    ...
    def __init__(
        ...
        if patch_size[0] == 2:
            ...
            self.proj = nn.Conv3d(
                in_chans,
                embed_dim,
                kernel_size=to_3tuple(kernel),
                stride=patch_size,
                padding=to_3tuple(pad))
...
def window_reverse(windows, window_size: int,D: int, H: int, W: int):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
class SpatialBlock(nn.Module):
    ...
    def forward(self, x, size): 
        ...
        attn_windows = attn_windows.view(-1,
                                         ...
                                         self.window_size,
                                         C)
        x = window_reverse(attn_windows, self.window_size,Dp, Hp, Wp)
...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, depths=(2, 2, 2, 2), patch_size=4,
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            D, H, W = sizes[i]
            out = x_out.view(-1, D, H, W, self.embed_dims[i]).permute(0, 4, 1, 2,3).contiguous()
            # print(out.shape)
            outs.append(out)
